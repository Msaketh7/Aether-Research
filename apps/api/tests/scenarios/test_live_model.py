...
from app.core.enums import LlmProvider, RunStatus
...
#: this test is to try a real model, and which one is a question worth asking.
...
def live_model(monkeypatch):
    ...
    def real_gateway(brain, *, settings, budget) -> LLMGateway:
        from app.models.providers.openai import OpenAIProvider

        # One real chat model serving every tier. The tiers exist to trade cost
        # against quality, and collapsing them here keeps the run cheap while
        # still exercising every role's prompt and parser.
        chat = {
            tier.value: ModelSpec(
                ...
                # A placeholder price, so the ledger has arithmetic to do and
                # the budget guard is exercised. Not a claim about what the
                # vendor charges - see the registry's pricing rules.
                pricing=Pricing(1.0, 2.0, dt.date(2026, 9, 19), "placeholder, not a vendor price"),
            )
            for tier in ModelTier
        }
        # The scripted embedding model, at the width the column declares. A real
        # one here would be refused: the hosted models are wider than 768, which
        # `ChunkEmbedder` correctly rejects, and the dense arm is dead without
        # pgvector anyway.
        ...
        return LLMGateway(
            ...
            # The guard is both recorder and budget, exactly as the worker wires
            # it: that is what enforces a ceiling before a call rather than
            # noticing it after the node that made it returned.
            recorder=budget,
            budget=budget,
    ...
    monkeypatch.setattr(world, "gateway_over", real_gateway)
