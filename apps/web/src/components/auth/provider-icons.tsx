/**
 * Brand marks for the SSO connections.
 *
 * Neither mark is redrawn. Google's is the four-path `logo_googleg_48dp` asset
 * served from Google's own CDN (gstatic), and GitHub's is simple-icons' path,
 * copied verbatim: a hand-authored approximation of somebody else's logo is
 * both wrong and a trademark problem. simple-icons is CC0, and the marks
 * themselves stay the property of their owners - see THIRD_PARTY_NOTICES.md.
 *
 * Google is in full colour, which is what Google's own identity guidelines ask
 * for on both light and dark surfaces. It is a logo beside a visible text
 * label, not text and not the only indicator of anything, so the 4.5:1 text
 * rule does not apply to it.
 *
 * GitHub publishes exactly two permitted colours for the Invertocat - its
 * near-black and pure white - so it takes the one that is legible against the
 * current theme rather than inheriting the button's muted foreground.
 */
