...
**All 25 phases are complete, plus Phase 26 (single sign-on) and Phase 27 (the streamed answer).** Full plan and per-phase status:
...
| Agents (`app/agents`)                 | All ten: planner, three researchers behind a router, evidence, claim normalization, verification, contradictions, critic, **the answerer**, synthesis, and a citation validator that calls no model. The answerer is the only one that streams: it writes the direct answer before the report, its text arrives as the reader watches, and a failure costs the answer rather than the run (ADR 0023). Versioned prompts as package data; a model cites by catalogue number and never emits an identifier (ADR 0015).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Answer (`app/answers`)                | The question, answered in four paragraphs before the report is written, and stored so a reader who was not watching gets the same text. `/answer` serves it - `null` until the run has written one, never a 404, because "not yet" is the normal state of a run that started ten seconds ago.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Streaming (`app/research`)            | The events the worker emits, carried across processes: `eventbus.py` is Redis pub/sub with a capped buffer over a durable log; `events.py` is the vocabulary and the in-process transport. `publish` numbers an event by storing it, so two emitters can never share an `id:` (ADR 0018). The answer travels the same way and not by a second channel: `answer_delta` is an ordinary stored row, which is what makes a reconnect replay the answer exactly (ADR 0023).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| [`docs/ADRs/`](docs/ADRs/)                     | 23 decisions (0021 superseded by 0022). New irreversible choice ⇒ new ADR. |
...
apps/api/          FastAPI + worker, one codebase two process types (ADR 0001)
  ...
  app/agents/      the research graph (state, node contracts, loop control,
                   checkpointer, runner) and the ten agents that fill it:
                   planner, researchers/ (web, documents, data, router),
                   extraction, verification, critic, answer, synthesis,
                   citations. answer.py is the only one that streams, and
                   nodes.py declares the AnswerStream it writes to.
  ...
  app/answers/     the direct answer as a row: schemas.py is what /answer
                   serves, repository.py the store's protocol and the derived
                   id, projection.py the write. No gate on a citation check -
                   the answer is what the reader was already shown (ADR 0023).
...
- **A few tests make real DNS and HTTP calls.** The SSRF guard's allow-path test
  ...
  `httpx2.MockTransport` - but the guard resolves every host _before_ the mock
  transport is reached, so a DNS blip fails `tests/test_research_tools.py`
  **wholesale**, thirty-nine tests at once, with `UrlRefused: That host could not
be resolved`. It looks like a broken toolbelt and is a broken network. Re-run
  the module alone before believing it; it passes in seconds when DNS is up.
